...
function clearMindmapData() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        if (
            ...
            key.startsWith('mindmap-node-green-') ||
            key.startsWith('mindmap-node-pink-') ||
...
async function replayPendingSyncs() {
    ...
    for (const localIdStr of ids) {
        ...
        try {
            ...
            const pk   = localStorage.getItem('mindmap-node-pink-' + localId);
    ...
    for (const m of maps) {
        ...
        if (m.data) {
            ...
            try {
                ...
                if (m.data._pink)      localStorage.setItem('mindmap-node-pink-'      + localId, JSON.stringify(m.data._pink));
